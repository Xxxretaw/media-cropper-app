...
- 每个视频都可单独设为跳过黑边检测，“检测全部”只处理未跳过的视频。
...
黑边检测默认按需执行。批量导入后，可逐个选择不需要处理的视频并勾选“当前视频跳过黑边检测”，再使用“检测全部”处理其余视频。检测会在所选输出片段的多个时间点采样，并综合 FFmpeg `bbox` 亮度边界结果。只有多数采样结果稳定一致时才会自动更新裁切框；检测到画幅变化时会保留当前裁切区域并标记“需确认”。检测成功后，该区域会成为后续比例、锚点、缩放和手动拖拽的有效边界，避免重新带回黑边。修改视频输出时间范围后，需要重新检测。
...
- `src-tauri/target/release/bundle/dmg/media-cropper_0.4.2_aarch64.dmg`
